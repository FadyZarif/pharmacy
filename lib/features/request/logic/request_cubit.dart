...
class RequestCubit extends Cubit<RequestState> {
  ...
  Future<String> uploadFileAndGetUrl(PlatformFile pickedFile) async {
    try {
      ...
      // If bytes not available (shouldn't happen with withData: true), try reading from path on mobile
      ...
      final snapshot = await ref.putData(
        bytes,
        SettableMetadata(contentType: pickedFile.extension == 'pdf' ? 'application/pdf' : 'image/${pickedFile.extension}'),
      );
  ...
  Future<void> approveRequest(RequestModel request) async {
    try {
      ...
      // Authorization:
      // - Management can approve/reject
      // - SubManager can approve/reject only if permission is enabled
      final canManage = currentUser.isManagement ||
          (currentUser.role == Role.subManager && currentUser.hasRequestsPermission);
      if (!canManage) {
        emit(AddRequestFailure(error: 'You are not authorized to approve requests.'));
        return;
      }

      // SubManager cannot approve/reject their own requests
      if (currentUser.role == Role.subManager &&
          currentUser.hasRequestsPermission &&
          request.employeeId == currentUser.uid) {
        emit(AddRequestFailure(error: "You can't approve your own request."));
        return;
      }
  ...
  Future<void> rejectRequest(RequestModel request) async {
    try {
      ...
      final canManage = currentUser.isManagement ||
          (currentUser.role == Role.subManager && currentUser.hasRequestsPermission);
      if (!canManage) {
        emit(AddRequestFailure(error: 'You are not authorized to reject requests.'));
        return;
      }
      ...
      if (currentUser.role == Role.subManager &&
          currentUser.hasRequestsPermission &&
          request.employeeId == currentUser.uid) {
        emit(AddRequestFailure(error: "You can't reject your own request."));
        return;
      }
