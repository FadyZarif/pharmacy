...
| Legal pages | `web_legal/`, deployed with `firebase deploy --only hosting`, live at <https://pharmacy-employee-system-new.web.app>. Linked in-app below the sign-in form and the branch picker — guideline 5.1.1(i) requires the policy to be reachable *inside* the app. |
| Force update | `ios_minimum_build_number` on iOS, `minimum_build_number` on Android. The field used to be shared, which would have locked every iPhone user out the moment an Android minimum was set. |
